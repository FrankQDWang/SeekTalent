...
LIEPIN_ENV_TEMPLATE_KEYS = [
    ...
    "SEEKTALENT_LIEPIN_OPENCLI_SEARCH_NAVIGATION_TIMEOUT_SECONDS",
    ...
    "SEEKTALENT_LIEPIN_EXPLOIT_DETAIL_TARGET",
    "SEEKTALENT_LIEPIN_EXPLORE_DETAIL_TARGET",
...
def _set_workbench_domi_node(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_workbench_command_runs_packaged_frontend_in_prod(
    ...
    argv, env = calls[0]
    _assert_workbench_server_launch(argv)
...
def test_workbench_command_launches_ui_server_with_current_python(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    home = tmp_path / "home"
...
def test_workbench_command_does_not_check_opencli_extension_during_startup(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    captured = capsys.readouterr()
    assert "warning_code=liepin_opencli_extension_disconnected" not in captured.err
    assert restart_calls == []
    assert stop_calls == []
    _assert_workbench_server_launch(launch_calls[0])


def test_workbench_command_does_not_supervise_opencli_daemon_from_startup(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("HOME", str(tmp_path / "home"))
...
def test_workbench_command_does_not_restart_daemon_when_status_fails(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("HOME", str(tmp_path / "home"))
...
def test_optional_runtime_env_vars_use_new_text_llm_keys() -> None:
    ...
    assert "SEEKTALENT_WORKBENCH_CONVERSATION_MODEL_ID" in OPTIONAL_RUNTIME_ENV_VARS
    assert "SEEKTALENT_WORKBENCH_CONVERSATION_REASONING_EFFORT" in OPTIONAL_RUNTIME_ENV_VARS
    assert "SEEKTALENT_CONTROLLER_TIMEOUT_SECONDS" in OPTIONAL_RUNTIME_ENV_VARS
...
def test_run_rejects_mock_cts_flag(capsys: pytest.CaptureFixture[str]) -> None:
    status = main(["run", "--job-title", "Python Engineer", "--jd", "JD", "--mock-cts"])
