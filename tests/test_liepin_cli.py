...
def test_liepin_smoke_pipeline_report_uses_runtime_artifact_fields() -> None:
    report = cli._liepin_smoke_pipeline_report(_fake_pipeline_artifacts())
...
def test_liepin_smoke_pipeline_runs_runtime_liepin_only(monkeypatch, capsys, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(cli, "WorkflowRuntime", FakePipelineRuntime, raising=False)
...
def test_liepin_smoke_pipeline_accepts_detail_backed_degraded_finalization(
    ...
    monkeypatch.setattr(cli, "build_liepin_worker_client", lambda settings: worker, raising=False)
    monkeypatch.setattr(cli, "WorkflowRuntime", RuntimeWithDegradedFinalization, raising=False)
...
def test_liepin_smoke_pipeline_reports_runtime_source_lane_safe_reason(
    ...
    monkeypatch.setattr(cli, "build_liepin_worker_client", lambda settings: worker, raising=False)
    monkeypatch.setattr(cli, "WorkflowRuntime", RuntimeWithBlockedSourceLane, raising=False)
...
def test_liepin_smoke_pipeline_rejects_missing_detail_evidence(monkeypatch, capsys, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(cli, "build_liepin_worker_client", lambda settings: worker, raising=False)
    monkeypatch.setattr(cli, "WorkflowRuntime", RuntimeWithoutDetailEvidence, raising=False)
...
def test_liepin_smoke_pipeline_rejects_empty_final_shortlist(monkeypatch, capsys, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(cli, "build_liepin_worker_client", lambda settings: worker, raising=False)
    monkeypatch.setattr(cli, "WorkflowRuntime", RuntimeWithoutFinalCandidates, raising=False)
