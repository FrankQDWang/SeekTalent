...
from experiments.openclaw_baseline import OPENCLAW_AGENT_ID, OPENCLAW_GATEWAY_BASE_URL, OPENCLAW_MAX_ROUNDS
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m experiments.openclaw_baseline.run",
        description="Run the OpenClaw baseline against the same CTS corpus and judge setup.",
    )
    ...
    parser.add_argument("--output-dir", default="runs/openclaw", help="Directory where run artifacts should be written.")
    parser.add_argument("--json", dest="json_output", action="store_true", help="Emit a single JSON object.")
    parser.add_argument(
        "--gateway-base-url",
        default=os.environ.get("OPENCLAW_BASE_URL", OPENCLAW_GATEWAY_BASE_URL),
        help="OpenClaw gateway base URL.",
    )
    parser.add_argument(
        "--agent-id",
        default=os.environ.get("OPENCLAW_AGENT_ID", OPENCLAW_AGENT_ID),
