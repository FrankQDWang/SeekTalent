...
_LIEPIN_FAILURE_POLICY_ENTRIES = (
    ...
    _policy(
        "liepin_cards_source_port_missing",
        "source_provider_failed",
        legacy_lane_retryable_metadata=False,
    ),
    _policy(
...
def public_source_failure_cause_code(
    source_kind: object,
    reason_code: object,
) -> str | None:
    source = str(getattr(source_kind, "value", source_kind or "")).strip()
    resolver = _PUBLIC_FAILURE_CAUSE_RESOLVERS.get(source)
    return resolver(reason_code) if resolver is not None else None
...
_PUBLIC_FAILURE_CAUSE_RESOLVERS = {
    "liepin": public_liepin_failure_cause_code,
}
