...
class _SidecarProcess:
    ...
    cards_session: PostHandshakeLiepinCardsSession | None
...
class _HistoryUnknown:
    ack: LiepinCardsAcceptedAckV1 | None
...
class LiepinCardsSourceOperationExecutor:
                ...
                for fact in result.facts:
                    if isinstance(
                        ...
                        recovered_ack = LiepinCardsAcceptedAckV1(
                            ...
                            ack_kind=(
                                "new_logical_operation"
                                if accepted.dispatch.dispatch_authorization_ordinal
                                == 1
                                else "new_dispatch_authorization"
                            ),
                            dispatch_intent_ref=(
    ...
    def _ready_cards_process(self) -> _SidecarProcess:
        if (
...
def _spawn_sidecar(
    *,
    ...
    cards_session = (
        None
        if history_only
        else PostHandshakeLiepinCardsSession(
            role="main",
