"""Boundary-neutral contracts for durable Liepin cards operations."""
...
class LiepinCardsArtifactV1(_CardsModel):
    ...
    canonical_request_hash: Annotated[
        str,
...
def canonical_liepin_cards_request_bytes(
...
def canonical_liepin_cards_request_hash(
    request: LiepinCardsOperationRequestV1,
) -> str:
    return sha256(canonical_liepin_cards_request_bytes(request)).hexdigest()


def stable_liepin_cards_operation_id(
    request: LiepinCardsOperationRequestV1,
) -> str:
    if type(request) is not LiepinCardsOperationRequestV1:
        raise TypeError("request must be LiepinCardsOperationRequestV1")
