...
def _is_provider_candidate_key_hash(value: object) -> TypeGuard[str]:
    return isinstance(value, str) and re.fullmatch(r"[0-9a-f]{64}", value) is not None


@dataclass(frozen=True, slots=True)
class _SidecarDetailObservation:
    """Sidecar-owned detail probe result; no main artifact is written."""
...
def _is_exact_liepin_host_candidate(candidate: BrowserHostTab) -> bool:
    parsed = urlparse(candidate.url)
    return (
        parsed.scheme == "https"
        and parsed.hostname == "h.liepin.com"
...
def _search_result_fingerprint(text: str) -> str | None:
    cards = extract_liepin_card_summaries(text, max_cards=50)
    if cards:
        material: object = cards
    else:
        targets = _rank_liepin_detail_targets(text, max_cards=50)
        if not targets:
            return None
        material = tuple(" ".join(target.block_text.split()) for target in targets)
...
class LiepinSiteAdapter:
    ...
    def _select_liepin_host_tab(self) -> BrowserHostTab:
        try:
            candidates = self._automation.find_host_tabs("https://h.liepin.com/")
        except OpenCliBrowserError as exc:
            raise liepin_error_from_opencli_error(exc) from exc
        safe_candidates = tuple(
            candidate
            for candidate in candidates
            if _is_exact_liepin_host_candidate(candidate)
        )
        windows = {candidate.window_id for candidate in safe_candidates}
        if not windows:
            raise OpenCliBrowserError("liepin_host_tab_missing")
        if len(windows) == 1:
            selected_window = next(iter(windows))
        else:
            focused_windows = {
                candidate.window_id for candidate in safe_candidates if candidate.window_focused
            }
            if len(focused_windows) != 1:
                raise OpenCliBrowserError("liepin_host_window_ambiguous")
            selected_window = next(iter(focused_windows))
        window_candidates = tuple(
            candidate for candidate in safe_candidates if candidate.window_id == selected_window
        )
        active_candidates = tuple(candidate for candidate in window_candidates if candidate.active)
        if len(active_candidates) == 1:
            return active_candidates[0]
        return min(window_candidates, key=lambda candidate: (candidate.url, candidate.page_id))
    ...
    def wait_liepin_detail_ready(self, *, source_run_id: str, rank: int) -> OpenCliBrowserResult:
        if self._cards_operation_executor is not None:
            raise RuntimeError("liepin_details_direct_browser_forbidden")
        return self._wait_liepin_detail_ready_unchecked(
            source_run_id=source_run_id,
            rank=rank,
        )

    def _wait_liepin_detail_ready_unchecked(
        self,
        *,
        source_run_id: str,
        rank: int,
    ...
    def _observe_liepin_detail_resume_for_sidecar(
        self,
        *,
        rank: int,
        expected_provider_candidate_key_hash: str | None,
    ) -> _SidecarDetailObservation:
        """Probe one detail resume without writing any main product projection."""
        try:
            if rank < 1 or rank > 100:
                raise OpenCliBrowserError("liepin_opencli_forbidden_command")
            detail_payload_text = self._run_fixed_readonly_eval_probe(
            ...
            if expected_provider_candidate_key_hash is not None:
                if source_url is None or not _is_provider_candidate_key_hash(
                    expected_provider_candidate_key_hash
                ):
                    raise OpenCliBrowserError("liepin_opencli_candidate_identity_mismatch")
                observed_key_hash = stable_liepin_detail_candidate_key_hash(source_url)
                canonical_source_url = canonical_liepin_detail_source_url(source_url)
                if (
                    observed_key_hash != expected_provider_candidate_key_hash
                    or canonical_source_url is None
                ):
                    raise OpenCliBrowserError("liepin_opencli_candidate_identity_mismatch")
                detail_payload["sourceUrl"] = canonical_source_url
                resume["provider_candidate_key_hash"] = observed_key_hash
                resume["claim_aware"] = True
            elif source_url is not None:
                detail_payload["sourceUrl"] = source_url
            resume["detail_payload"] = detail_payload
            return _SidecarDetailObservation(resume=resume, safe_reason_code=None)
        except OpenCliBrowserError as exc:
            return _SidecarDetailObservation(
                resume=None,
                safe_reason_code=exc.safe_reason_code,
            )
    ...
    def _ingest_liepin_detail_resume_from_source_artifact(
        ...
        *,
        ...
        """Write main-owned product projections for one validated details resume."""
        if rank < 1 or rank > 100:
            raise OpenCliBrowserError("liepin_opencli_forbidden_command")
        safe_run_id = _safe_artifact_segment(source_run_id)
        detail_payload = resume.get("detail_payload")
        normalized_text = resume.get("normalized_text")
        page_url_hash = resume.get("page_url_hash")
        if (
            not isinstance(detail_payload, Mapping)
            or not isinstance(normalized_text, str)
            or not (page_url_hash is None or isinstance(page_url_hash, str))
        ):
        ...
        projected: dict[str, object] = {
            ...
            "protected_snapshot_ref": raw_snapshot_ref,
            "normalized_snapshot_ref": normalized_snapshot_ref,
            "detail_payload": {str(key): value for key, value in detail_payload.items()},
            "normalized_text": normalized_text,
        }
        if resume.get("claim_aware") is True:
            provider_candidate_key_hash = resume.get("provider_candidate_key_hash")
            if not _is_provider_candidate_key_hash(provider_candidate_key_hash):
                raise OpenCliBrowserError("liepin_opencli_candidate_identity_missing")
            projected["provider_candidate_key_hash"] = provider_candidate_key_hash
            projected["claim_aware"] = True
        else:
            projected["provider_candidate_key_material_ref"] = self._write_pi_artifact(
                "protected",
                f"liepin-opencli/provider-key/{safe_run_id}/{rank}.txt",
                _detail_provider_key_material(
                    safe_run_id=safe_run_id,
            ...
            projected["candidate_resume_id"] = (
                f"liepin-opencli-detail-{safe_run_id}-{rank}"
            )
        self._upsert_collected_resume(safe_run_id, rank=rank, resume=projected)
    ...
    def _execute_liepin_details_sidecar_effect(
        ...
        *,
        ...
        if open_mode not in {"resolve_locator", "cached_locator"}:
        ...
        try:
            ...
            wait_result = self._wait_liepin_detail_ready_unchecked(
                source_run_id=source_run_id,
                rank=rank,
            )
            ...
            observed = self._observe_liepin_detail_resume_for_sidecar(
                rank=rank,
                expected_provider_candidate_key_hash=(
                    expected_provider_candidate_key_hash
                ),
            )
            if observed.resume is None:
                return _details_sidecar_effect_result(
                    status="failed",
                    provider_candidate_key_hash=provider_candidate_key_hash,
                    detail_url=locator.detail_url,
            ...
            return _details_sidecar_effect_result(
                status="succeeded",
                provider_candidate_key_hash=provider_candidate_key_hash,
                detail_url=self._current_liepin_detail_url() or locator.detail_url,
    ...
    def _search_liepin_cards_once(
        ...
        *,
        ...
        try:
            ...
            current_query = self._liepin_search_query_value_from_dom(ref=search_input_ref)
            reuse_current_results = (
                _search_query_matches(current_query, query)
                and _search_result_fingerprint(first_state_text) is not None
                and _search_state_ready_for_card_extraction(_snapshot_from_result(first_state))
            )
            events.append(
                {
                    "action_kind": (
                        "reuse_current_search_results"
            ...
            fill_result = self._run_liepin_transition(
                LiepinTransition(
                    name="fill_search",
                    phase="search",
                    observe_pre_state=lambda: _snapshot_from_result(first_state),
                    precondition=lambda snapshot: snapshot.ok,
            ...
            pre_click_result_fingerprint = _search_result_fingerprint(click_ready_state_text)
            if not reuse_current_results:
            ...
            def observe_after_click_search() -> LiepinStateSnapshot:
                ...
                if reuse_current_results:
                    click_post_snapshot = _snapshot_from_result(first_state)
            ...
            click_result = self._run_liepin_transition(
                LiepinTransition(
                    name="click_search",
                    phase="search",
                    observe_pre_state=lambda: _snapshot_from_result(click_ready_state),
                    precondition=lambda snapshot: snapshot.ok,
            ...
            def observe_results_post_state() -> LiepinStateSnapshot:
                ...
                while True:
                    try:
                        ...
                        time.sleep(_SEARCH_URL_POLL_SECONDS)
                        continue
                    events.append(
                        {
                            "action_kind": "observe_results",
                            "route_kind": "search",
                    ...
                    if observed_state.ok:
                        ...
                        if _search_state_ready_for_card_extraction(snapshot):
                            result_fingerprint = _search_result_fingerprint(snapshot.text)
                            if (
                                reuse_current_results
                                or pre_click_result_fingerprint is None
                                or result_fingerprint != pre_click_result_fingerprint
                            ):
                                fresh_input_ref = extract_liepin_search_input_ref(snapshot.text)
                                if fresh_input_ref is None:
                                    return LiepinStateSnapshot(
                                        ok=False,
                                        text=snapshot.text,
                                        url=snapshot.url,
                                ...
                                return snapshot
                            last_retry_reason = "liepin_opencli_results_not_ready"
                            if not stale_result_observed:
                                stale_result_observed = True
                                events.append(
                                    {
                                        "action_kind": "observe_stale_results",
                                        "route_kind": "search",
                                        "safe_reason_code": last_retry_reason,
                                    }
                                )
                    ...
                    elif observed_state.safe_reason_code == "liepin_opencli_status_unavailable":
