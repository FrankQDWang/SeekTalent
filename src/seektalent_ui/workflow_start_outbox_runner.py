...
class WorkflowStartOutboxRunner:
    def __init__(
        self,
        *,
    ...
    def start(self) -> None:
        with self._lock:
            ...
            self._thread = threading.Thread(
                ...
                name="seektalent-wts-workflow-start-outbox-runner",
                daemon=True,
            )
            self._thread.start()
    ...
    def run_once(self) -> int:
        ...
        candidates = self.service.outbox_store.list_claimable_workflow_start_items(
        ...
        for item in candidates:
            ...
            if item.status == "pending" and not _retry_due(item.updated_at, attempt_count=item.attempt_count, now=now):
                continue
            try:
                self.service.process_workflow_start_outbox_item(item.outbox_id)
            except (ConversationAgentError, RuntimeControlError, sqlite3.Error):
                logger.exception("WTS workflow start outbox item failed.", extra={"outbox_id": item.outbox_id})
