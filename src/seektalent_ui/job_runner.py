...
from seektalent_ui.workbench_note_writer import WorkbenchNoteWriter
from seektalent_ui.runtime_bridge import RuntimeFactory, run_cts_source_run, run_liepin_card_source_run
from seektalent_ui.workbench_store import WorkbenchSourceRunJobContext, WorkbenchStore, WorkbenchUser, _iso, _now
...
class WorkbenchJobRunner:
    ...
    def _tick_note_writer(self, context: WorkbenchSourceRunJobContext) -> None:
        ...
        try:
            self.note_writer.tick_session(user=user, session_id=context.session.session_id)
