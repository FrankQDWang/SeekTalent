...
class ResumeNormalizer:
    ...
    def _normalize_strings(self, values: list[Any]) -> list[str]:
        cleaned = []
        for value in values:
        ...
        if isinstance(raw_items, list):
            for item in raw_items[:4]:
                ...
                experience = NormalizedExperience(
                    ...
                    company=self._first_text(item.get("company")),
                    duration=self._first_text(item.get("duration"), item.get("startTime")),
                    summary=self._first_text(item.get("summary")),
                )
    ...
    def _extract_skills(
        ...
        *,
        ...
        skills = self._normalize_strings(explicit)
        if skills:
            return skills[:12]
    ...
    def _extract_industry_tags(self, candidate: ResumeCandidate) -> list[str]:
        ...
        return self._normalize_strings(raw_values)[:8]

    def _extract_language_tags(self, raw: dict[str, Any]) -> list[str]:
        values: list[Any] = []
        for key in ("languageTags", "languages"):
            value = raw.get(key)
            if isinstance(value, list):
                values.extend(value)
        return self._normalize_strings(values)[:8]
            ...
            for item in raw_items[:2]:
                ...
                summary.append(
                    " ".join(
                        part
                        for part in [
                            self._first_text(item.get("school")),
                            self._first_text(item.get("speciality")),
                            self._first_text(item.get("degree")),
                        ]
                        if part
                    )
                )
            return " ; ".join(self._normalize_strings(summary))
        return ""
    ...
    def normalize(self, candidate: ResumeCandidate) -> NormalizedResume:
        ...
        candidate_name = self._extract_name(raw)
        current_title = self._first_text(
            raw.get("current_title"),
            raw.get("currentTitle"),
            raw.get("title"),
            candidate.expected_job_category,
        )
        current_company = self._first_text(
            raw.get("current_company"),
            raw.get("currentCompany"),
            raw.get("company"),
        )
        recent_experiences = self._extract_recent_experiences(candidate, normalization_notes=normalization_notes)
        if not current_company and recent_experiences:
            current_company = self._first_text(recent_experiences[0].company)
            if current_company:
                normalization_notes.append("Filled current_company from the first recent experience.")
        headline = self._first_text(raw.get("headline"))
        ...
        locations = self._normalize_strings(
            [
        ...
        education_summary = self._extract_education_summary(candidate, normalization_notes=normalization_notes)
        skills = self._extract_skills(candidate, normalization_notes=normalization_notes)
        industry_tags = self._extract_industry_tags(candidate)
        language_tags = self._extract_language_tags(raw)
        key_achievements = self._build_key_achievements(
            candidate,
            recent_experiences,
            normalization_notes=normalization_notes,
        )
        raw_text_excerpt = self._build_excerpt(
            candidate,
            recent_experiences,
            normalization_notes=normalization_notes,
        )
        completeness_score, missing_fields = self._completeness_score(
