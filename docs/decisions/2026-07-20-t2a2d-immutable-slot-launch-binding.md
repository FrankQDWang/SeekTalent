...
Status: conditional Windows bridge; macOS writable-slot pre-spawn no-go; production-unreachable
...
3. macOS has no supported CPython/subprocess fd-exec route; ordinary FDs, `flock`, and `O_EXLOCK` are advisory. The pre-spawn security guarantee is therefore unavailable for a writable per-user slot. A raw concurrent path-start/replace observation on macOS 15.6.1 arm64 left a `_dyld_start` child unreapable after `SIGKILL`, so that race is also a no-go as required CI automation—not evidence that the race is safe. The v1 reliability boundary is immutable concrete slots plus a cooperative lifecycle lease; strict same-UID hostile-process protection requires installer ownership/permissions and later signed-PID evidence.
...
The deterministic probe has no production import or caller and runs only on the real host. Its stable JSON records architecture, OS release/build, and Python implementation/version/build with each evidence artifact:

| Host | Required evidence | Current result |
| --- | --- | --- |
| macOS 15 arm64 | Deterministic FD/path replacement red reproducer; `flock` limits; cooperative activation/rollback/delete lease; suspended Security.framework dynamic requirement | Raw concurrent path race is explicit no-go for CI after one unreapable `UE` child; deterministic rows remain required |
| macOS 15 Intel x86_64 | Same deterministic Darwin rows on native Intel, never Rosetta or `os.name` simulation | Required GitHub Actions row |
| Windows Server 2025 x64 | Local NTFS, reparse rejection, complete component-plus-leaf share-deny lease, suspended `CreateProcessW`, `FILE_ID_INFO`/final-handle binding, bounded 1,000 race and cleanup | Required GitHub Actions row |
...
The Windows probe holds every synthetic installed-root/slot/release/bin component handle and the payload handle through identity gates, then terminates and reaps every child while its primary thread remains suspended. It rejects reparse components, requires local NTFS, verifies the raw process image path only by reopening it and comparing `FILE_ID_INFO` plus final handle path, and records a bounded 1,000-start/replace error histogram. A later implementation resumes only after its platform evidence and then still withholds workflow authority until `MainReady`.

The macOS raw non-cooperating path race is deliberately absent from the required CI job. One local arm64 attempt left a `_dyld_start` child as PPID 1 / `UE` after `SIGKILL`; neither a process group nor a supervisor can make that uninterruptible kernel wait safely cleanable. This confirms the per-user writable-slot pre-spawn no-go. The deterministic `Popen` replacement reproducer, cooperative lifecycle lease, and suspended Security.framework PID check remain the native evidence this PR can safely require.

Native evidence is split by host ownership: `scripts/verify-native-macos-arm64.sh` runs the Apple Silicon row locally before publishing relevant delivery changes, while `.github/workflows/native-launch-binding-probe.yml` pins `windows-2025` and `macos-15-intel` for hosts unavailable to the developer machine. The combined three-host evidence remains required while Intel is supported, but only the two unavailable hosts run automatically on matching pull requests. GitHub has announced Intel runner retirement after Fall/August 2027; before that sunset, product owners must either move the Intel row to owned/self-hosted hardware or explicitly retire Intel support. GitHub VMs prove API semantics, not Windows 10/11 installer behavior, SmartScreen, macOS ownership/quarantine, Gatekeeper, notarization, or clean-user release acceptance.
...
| Normal product process starts a new write/delete/rename | Covered for all simultaneously lease-held local NTFS components and leaf | Covered only when installer ownership denies it | In scope as stated |
| Non-cooperating same-UID process | Limited to Windows objects protected by complete share-deny coverage; a pre-existing writer yields `executable_share_mode_conflict` and no child, while destructive denial of service remains possible | Explicit writable-slot pre-spawn no-go; raw race is unsafe to automate | Never claim updater locking is a security boundary |
...
| macOS same-UID non-cooperating pre-spawn race | Explicit no-go for a per-user writable slot; do not automate raw concurrent races after the unreapable-child observation. Installer ownership plus suspended signed-PID evidence remains required; production remains unreachable. |
