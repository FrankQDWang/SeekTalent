...
### `browser_control_lanes`

| 字段 | 语义 |
|---|---|
| `lane_key_hash` | 不暴露原始账号信息的稳定 lane 标识 |
| `current_scope_id` | 当前有命令权的 scope |
| `current_fence_token` | extension 返回的当前 fencing token 镜像 |
| `updated_at` | 最近一次 scope 激活时间 |
...
| `source_kind` | `liepin` 等数据源 |
| `source_run_id` | 业务来源；不可作为所有权凭证 |
| `runtime_run_id` | 可选业务追踪字段 |
| `source_lane_run_id` | 可选业务追踪字段 |
| `owner_instance_id` | 本次 SeekTalent 进程启动 ID |
...
| `state` | `allocating | owned | reclaim_requested | reclaim_failed | reclaimed` |
| `created_at` | 创建意图落库时间 |
...
tab:
  allocating
    ...
    -> reclaimed           extension/session 证明 tab 不存在
  owned
  ...
  reclaim_failed
    -> reclaimed           后台重试或 idle alarm 最终成功
```
...
scope 使用 `try/finally`，但 finally 中禁止运行同步 OpenCLI close：

1. 尽力把 scope/tab 标为 `reclaim_requested`；
2. 尽力向本地后台 reclaimer 发出幂等唤醒；
3. 立即返回原业务结果或继续传播原异常/取消。

后台 reclaimer 独立遍历该 scope 的精确 session/page 并请求关闭。它可以串行或小并发执行，但不在 source run 的关键路径中。
...
| 精确 close 失败 | 只写 cleanup warning | 保持锁定，后台重试/alarm 兜底 | 不受影响 |
...
- 新建聚焦生命周期的模块，例如 `src/seektalent/opencli_browser/lifecycle.py`：可降级的 SQLite registry 镜像、scope 激活结果、后台 reclaim；它持有真实状态，使用类是合理的。cleanup/record/telemetry 入口必须 non-throwing。
