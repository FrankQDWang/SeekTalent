...
- Failure Envelope schema builder/validator、immutable revision 分配，以及 receipt/evidence revision；
- journal compaction、retention、emergency ring；
...
`needs_attention/failed` 引用的最小 canonical Failure Envelope revision 是唯一的同步例外：main runtime 决定业务提交时机，调用 diagnostics service 提供的 schema builder/validator，并让该模块使用 main 已打开的 SQLite transaction 写 immutable row。该 row 与 run state、`ProductOutcome`、failure reference 位于同一 main SQLite transaction domain；不得用另一 SQLite、事后补写或分布式事务模拟原子性。diagnostics service 仍拥有 schema/validation/row projection，main runtime 不得自行拼接 envelope；sidecar 和其他 producer 也不得写 main DB。
...
| `first_failure_event_id/last_observed_event_id` | 锚定 canonical event identity。通常 first 必填；若 event 尚未 append，必须由同一 main transaction 中的 projection-outbox intent 持有相同 event ID，不能预留空号。canonical sink 与 emergency ring 同时不可用时可为 null，但必须填写 `diagnostic_gap` 与 `observed_boundary_ref` |
...
对每个 `needs_attention` 或 `failed` commit，main 在一个 SQLite transaction 中完成以下不可分割写入：

1. #324 根据 durable business truth 选择 run state 与 `ProductOutcome`；
2. main 调用 #322-owned Failure Envelope schema builder/validator，以同一 transaction handle 同步写入一条最小 canonical immutable revision row；
3. 同步写入 run state、`ProductOutcome` 与指向该 row 的稳定 `failure_id + revision` reference。

这四项——state、outcome、minimal envelope revision row、failure reference——必须全有或全无。任何 statement、builder validation 或 row insert 失败都回滚整个 transaction；不得先提交 state/outcome/ref，再让 journal sink 异步补出 envelope。对应 revision 的 `current_outcome` 必须等于同一 transaction 已决定的 outcome；`needs_attention` revision 还必须带同一业务提交使用的 concrete `user_action`。未被业务 outcome 引用的早期事实 revision 可以让 `current_outcome=null`，但不能被用作 `needs_attention/failed` 的 current reference。

若最小 row 填写 `first_failure_event_id`，同一 main transaction 还必须写入带完全相同 ID 的 canonical-event projection outbox intent；journal consumer 可在 commit 后异步 append，但 event anchor 不是凭空预留。若该 anchor 为 null，只能使用 §5.1 已冻结的 sink + emergency-ring gap 规则，并同步保存 `diagnostic_gap` 与 main-owned `observed_boundary_ref`。

最小 row 只记录本文件 schema 允许的事实，不授权 retry、状态迁移或 outcome。#322 仍唯一拥有 schema、validation 与 privacy-safe projection；#324/main runtime 只拥有业务决策与 transaction timing。diagnostics service 以 main 内 transaction participant/module 参与同一个 SQLite commit，不独立打开第二个数据库或先后提交；sidecar 只提供 allowlisted facts，绝不能写该 row 或 main DB。

业务 commit 完成后，canonical journal event、`OperationEvidence`、support bundle/export 和其他扩展 projection 才可经 projection outbox 异步物化。journal sink 暂时不可用、`OperationEvidence` 缺失或 support export 失败，只会让扩展 projection pending/产生 diagnostic gap；不得回滚或改写已经完整提交的四项。这里的“不阻断”不适用于最小 Failure Envelope row 本身：它是业务 outcome 的同事务非悬空事实。Failure Envelope 始终不包含任何 retry permission。
...
sidecar/source facts
  ...
  -> one main SQLite transaction
       -> #322 builder/validator writes minimal Failure Envelope revision
       -> state + ProductOutcome + failure reference commit with it, all-or-nothing
  -> canonical-event/extended projection intent may commit in main outbox
  -> #322 canonical journal events/OperationEvidence/support projections
```

#325 可以传输或引用 evidence，但不能让 canonical journal 反向拥有 source/run 状态机；diagnostics service 也不能把 journal 中的事实直接提交为业务 truth。同步最小 envelope row 是 outcome transaction 的事实 participant，不是 journal 反向驱动业务状态。
...
4. canonical journal sink 不可用不能阻止已经由同一 main SQLite transaction 完整写入的 state、outcome、minimal Failure Envelope revision 和 reference。main projection outbox 保持 pending；恢复后 diagnostics service 可生成 `diagnostic_gap_detected` 并在 `OperationEvidence` 标记 gap。schema builder/validator 或最小 row insert 失败属于业务 transaction 失败，不得被误称为可忽略的 sink loss。
...
| F06 | result durable 后，在 `needs_attention/failed` main transaction 的每个 SQL statement 前后 kill | crash 后只能观察到旧 state/outcome 且无新 ref/row，或新 state + matching outcome + minimal envelope revision + non-dangling ref 四项同时存在；无 partial/dangling commit | PR SQLite failpoint + nightly |
| F07 | 四项 main commit 后、journal/event projection 或 cleanup 前 kill | state/outcome/minimal envelope/ref 重启后完整一致；projection outbox 可 pending，cleanup/projection failure 不改写四项 | nightly |
...
| F24 | canonical journal sink/ring 同时不可用 | 若业务进入 `needs_attention/failed`，同事务 minimal envelope/ref 仍完整；event ID 可由同事务 outbox intent 持有，若 `first_failure_event_id=null` 则必须带 diagnostic gap reason/counter、observed boundary ref；`OperationEvidence` 可缺失，无伪造 anchor/raw stderr fallback | nightly |
...
| Gate | 运行时机 | 内容 | 阻断条件 |
|---|---|---|---|
| PR deterministic | 每个相关实现 PR；可跨平台模拟 | schema/registry validation、identity/cause DAG、reason mapping、redaction golden/property、size/budget、deterministic bundle、stale fence、main SQLite 四项原子提交 failpoints、synthetic protocol faults | 任一 invariant、privacy scan、dangling failure ref、partial four-item commit 或 deterministic case 失败 |
...
1. **Schema PR**：只实现 canonical models/registry/redaction/domain mapping、Failure Envelope builder/validator 和 golden fixtures；不接 runtime writes。
2. **Minimal-envelope storage PR**：只在 main runtime-control SQLite transaction domain 增加 #322-owned immutable Failure Envelope revision persistence/module API；证明它接受 caller transaction handle，不打开第二个 SQLite，也不允许 sidecar 写入。
3. **Atomic outcome integration PR**：由 #324-owned main runtime 在 `needs_attention/failed` transaction 中调用上述 API，原子写 state + outcome + minimal envelope revision + ref；对每个 statement 做 crash/rollback failpoint，不接 journal/export。
4. **Journal storage PR**：实现唯一 main diagnostics journal sink、projection outbox ingest、`JournalAppendAck`、compaction、retention、emergency ring；证明 sink/event projection loss 不改写已完整提交的四项。
5. **Producer PRs**：按 runtime、Liepin/source、browser 分开接 adapter；每个 PR 只发 allowlisted facts，并有 stale authority/correlation negative tests。
6. **Doctor/export PR**：实现三类 canonical object、preview/manual export、untrusted re-import 和 managed-copy lifecycle；不改 browser execution。
7. **Fault-harness PRs**：先 main SQLite atomic boundary/process/storage，再 browser/MV3；建立 PR deterministic 与各 native OS nightly gates。
8. **Release integration PR**：只消费 #326 exact artifact refs，建立逐 declared platform clean-machine/canary verdict；不反向扩大发布平台承诺。
...
- `needs_attention/failed` 的 state、`ProductOutcome`、minimal Failure Envelope immutable revision 与 non-dangling ref 在同一 main SQLite transaction 全有或全无；journal/event/evidence/export 才允许异步；
...
- 每个 `needs_attention/failed` 路径在真实 main SQLite file 上通过 statement-level crash/rollback tests，证明四项原子性；journal sink 故障不得破坏已提交四项，builder/row 失败则必须回滚业务 transaction；
