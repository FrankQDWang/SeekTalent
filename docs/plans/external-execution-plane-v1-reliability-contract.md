...
- 状态：已批准；合并至 `main` 后生效（Issue #321）
- 适用范围：SeekTalent External Execution Plane v1 及其完整桌面发布物
- 上游决议：[#320](https://github.com/FrankQDWang/SeekTalent/issues/320)
- 下游任务：[#322](https://github.com/FrankQDWang/SeekTalent/issues/322)、[#323](https://github.com/FrankQDWang/SeekTalent/issues/323)、[#324](https://github.com/FrankQDWang/SeekTalent/issues/324)、[#325](https://github.com/FrankQDWang/SeekTalent/issues/325)、[#326](https://github.com/FrankQDWang/SeekTalent/issues/326)
...
v1 的默认支持方式是现有用户 profile compatibility mode：使用用户已有的 Chrome Stable 登录会话，但产品必须确定性绑定一个 profile、一个 production extension 实例和一个猎聘账号主体。该选择优先保证现有登录体验，并明确保留用户 profile、其他 extension、企业策略和 Chrome 生命周期带来的剩余环境风险。运行期间：
...
产品不得采集用户密码、自动绕过风控、把验证码外发，或用隐藏 profile 伪装登录。专用产品 profile 不进入 production 双轨实现，但在 v1 production release 前必须完成一次时间盒真实账号 spike，验证登录、风控、搜索、长会话、企业策略、extension 分发和迁移体验。spike 通过后由独立产品决议决定是否替换默认；未通过则保留 compatibility mode，并把剩余风险写入 release evidence。spike 必须执行，但 dedicated profile 本身不要求通过。
...
控制平面的 lifecycle state、单个 Source Operation 的 disposition 与产品 outcome 是三个维度，禁止继续把它们压缩为一个模糊 `success/failed` 字段：

- `SidecarLifecycleState` 使用封闭枚举 `stopped/starting/ready/busy/degraded/draining/crashed/bounded_restart`，只描述进程与能力生命周期；
- `SourceOperationDisposition` 使用封闭枚举 `completed/partial/user_action_required/incompatible/failed/cancelled/reconciliation_unknown`，只报告单次外部执行事实，并单独携带 retry/reconcile posture；`verify_session` 的 `ready/not_ready` 是 operation-specific readiness fact，不是所有 operation 共用的完成 disposition；
- `ProductOutcome` 只由主应用根据 durable run、source coverage 和 candidate truth 决定。

Sidecar 不得直接写 Product Outcome。`user_action_required` 只有在携带具体、可执行的用户动作时才映射为 `needs_attention`；restart budget 耗尽、协议损坏等纯基础设施故障默认映射为 typed `failed`，不能伪装成需要用户处理。
...
| `degraded_with_results` | 至少一个 required path 未完成，但已提交结果仍可信可用 | 已提交结果必须保留并标明 coverage | terminal；显式补跑创建关联 run |
| `needs_attention` | 可由用户动作解除的登录、风控、权限、账号/profile 选择等阻塞 | 已提交结果保留 | 用户动作后恢复 |
| `failed` | 运行无法在契约内完成，且不存在等待用户动作的安全恢复路径 | 已提交结果不得丢失，但不得伪装完整 | terminal；显式重跑创建关联 run |
| `cancelled` | 用户或系统明确取消 | 已提交 safe-boundary 结果保留并标明 cancelled coverage | terminal；显式重跑创建关联 run |
...
`succeeded_with_results`、`succeeded_empty`、`degraded_with_results`、`failed` 和 `cancelled` 都是 terminal Product Outcome；已经 terminal 的 run 不得重开。只有 non-terminal、仍可恢复的 run 可以因进程重启产生新 attempt。terminal 后的人工重跑或缺失 source 补跑必须创建新的关联 run；#324 负责冻结关联字段与状态转换，但不得改变 terminal immutability。
...
#323 负责拓扑、唯一 owner、trust boundary 和 transport 方向；#325 必须在该拓扑及 #324 状态语义之上冻结 wire contract。无论采用哪种本地 transport，协议都必须满足：

1. request/result/event 都是纯数据、显式版本化、可序列化的契约；
2. 不跨进程传递 Python callback、数据库连接、browser 对象、Pydantic 私有实现对象或任意 `object`；
3. 每个 request 包含 `contract_version`、`operation_id`、`run_id`、`source`、`request_hash`、`attempt_no`、`runtime_attempt_fence_token`、`profile_binding_generation`、`browser_control_scope_id`、`deadline`、`deadline_clock`、`deadline_unit`、`idempotency_key`；`browser_control_scope_id` 只是 correlation identity，不是 authority；browser activation 后才由 sidecar/extension 产生 controller-only `control_key + browser_control_fence_token`，初始 request 不得要求 main 伪造它们，后续 result/evidence 只能返回非敏感引用；
...
9. 所有 deadline 使用明确时基和单位，超时后旧执行者的 durable 写入由 `runtime_attempt_fence_token` 阻断，旧 profile/controller 的 browser 副作用分别由 `profile_binding_generation` 与 browser boundary 每条 command 验证的 `control_key + browser_control_fence_token` 阻断。

## 8. Failure Envelope 与用户可见错误

#322 唯一定义 canonical Failure Envelope、事件关联、receipt 和 support projection；#325 定义 Failure Envelope 如何经 Source Execution Port 传输、引用与协商版本。任何 `failed` 或 `needs_attention` Product Outcome 必须关联一个版本化 Failure Envelope，至少包含：
...
- `user_action`、`support_action`；
- 当前 outcome、source coverage、last safe boundary；
- `occurred_at`、redaction policy/version。

Failure Envelope 只记录诊断事实，不包含 `retryable`、`safe_to_retry` 或其他 retry permission。`RetryPosture` 是唯一执行权威：`reconcile_first` 必须先查询 sidecar journal，只有 reconciliation 证明未产生外部副作用且当前三种 authority 仍有效时，#324 才能允许 `safe_retry`；任何 Envelope 字段都不能覆盖该决定。
...
#322 负责冻结 canonical Failure Envelope/事件 schema、receipt、support bundle 和故障注入基线；#325 只能在 Source Execution Port 中传输或引用这些对象，不得另造第二套错误模型。本契约冻结其产品要求：
...
non-terminal logical run 因恢复产生的新 attempt 仍属于同一个 run；terminal 后的重跑创建关联的新 run。可靠性统计必须同时提供 logical-run 分母和 attempt 分母，且把关联重跑单独展示，避免通过自动重试美化结果。
...
纯文档 PR 至少运行 `git diff --check`、仓库现有 governance/privacy/agent-safety gate，以及可复现的本地相对链接和 Markdown table 校验；PR 必须记录实际执行命令，不得把未配置的 GitHub check 描述为已通过。代码 PR 不得因为路径过滤而跳过对应 gate。
...
夜间自动化使用 contract site/fixture，不依赖真实个人账号。真实猎聘测试账号的 smoke canary 另行人工执行，只记录 sanitized outcome，并作为 release gate 的必需输入；contract site 通过不能替代真实站点 canary。
...
8. 发布 canary 的首个 synthetic candidate 持久化、主应用重启后仍可见；
9. 最终 artifact、production extension 和受控真实猎聘测试账号完成一次非敏感查询，产生 source-executed evidence 及合法 outcome；失败或无法执行时该 artifact 只能 internal-only；
10. `succeeded_empty`、partial/degraded、needs-attention 和 typed failure 的 UI 验证；
11. doctor、support bundle、签名、manifest、receipt 完整性；
12. 卸载/回滚不删除用户数据，不影响非 owned tab；
13. 所有 release blocker 均有可追溯证据链接。
...
| Source seam 仍含进程内对象 | `src/seektalent/source_contracts/runtime_lanes.py` request 含 callback、domain object、宽泛 `object` | #323 固定进程边界；#325 在更低的 Liepin worker/provider seam 定义纯数据 IPC contract |
...
| Browser | Chrome Stable；v1 先采用现有用户 profile compatibility mode 的确定性绑定 | 最贴近当前真实登录路径，但明确保留用户环境差异 | production release 前完成专用 profile 真实账号 spike；通过后以独立决议决定是否替换，未通过则记录 compatibility residual risk |
...
| Owner | 尚待决定 | 已冻结的边界 |
| --- | --- | --- |
| #322 | canonical Failure Envelope/事件/receipt schema、support bundle JSON、CLI/UI 导出入口、故障注入 harness | 必须 local-only、allowlist、可预览、默认无敏感正文；是 #325 的故障证据输入 |
| #323 | sidecar supervisor/WTSCLI controlled-mode 的可实现前置、profile binding ownership、transport 方向 | 必须保持唯一 owner；不得以 detached/unowned daemon 冒充 supervised child |
| #324 | `needs_attention` 在现有 FSM/DB 的精确映射、safe boundary 列表、terminal rerun lineage、outbox/reconcile schema | 不得丢 accepted job；恢复必须保留 logical run 和 candidate truth；terminal run 不重开 |
| #325 | Source Execution Port wire DTO、三类 authority/fence、journal 原子性和 Failure Envelope 的传输/引用/version negotiation | 必须满足第 7 节；不得重定义 #322 canonical evidence 或序列化 `RuntimeSourceLane*` 领域对象 |
| #326 | 各平台安装器技术、签名证书、exact OS/Chrome version window、CI runner/clean-machine provider、dedicated-profile spike 与真实站点 canary | 必须产出完整 signed artifact、receipt、A/B rollback、三平台 evidence 和 production canary |
| 产品 + 工程（有基线后） | 现场 field SLO 初始阈值 | 必须使用第 11 节分母，不能用重试掩盖 logical-run failure |

若下游发现上述实现选择无法满足硬约束，应回到 #321 修改契约并记录证据；不得在实现中静默弱化发布标准。

## 16. 对 #323 / #324 / #325 / #326 的影响

### #323：冻结受监督 sidecar 拓扑与唯一 owner

#323 合入前必须证明：

- main/sidecar/WTSCLI/profile binding/owned-tab 的唯一 lifecycle owner；
- WTSCLI 能被 sidecar 以 foreground/serve mode 或等价 owned bridge 真实监督，不依赖 detached/unref daemon；
- main 与 sidecar 的 process/trust boundary、本地认证和 transport 方向；
- 用户拥有 Chrome/profile/host window，产品只拥有 binding control lock 与 owned tab；
- production hard cut 不存在 main/sidecar browser 双 owner；
- owned-tab 边界和 user-tab 不可侵犯。

任何继续由 main process 直接 restart daemon、持有 tab/page ref 或旁路 sidecar 的 production composition 都不满足 v1。
...
### #325：冻结 Source Execution Port 与故障证据 wire contract

#325 必须：

- 把 IPC seam 放在 main 内部 `RuntimeSourceLane*` 应用边界之下的 Liepin worker/provider boundary；
- 为当前 production `verify_session/search/cards/details/continuation/cleanup` 冻结 operation-specific 纯数据 DTO，禁止 arbitrary object/callback/private continuation；
- 完整继承第 7 节 identity、hash、deadline、idempotency、attempt/fence、query/cancel/reconcile/drain 约束；
- 区分 `runtime_attempt_fence_token`、`profile_binding_generation` 和 controller-only `control_key + browser_control_fence_token` 三种 authority；`browser_control_scope_id` 仅用于 correlation，禁止用一个模糊 `fence` 代替这些字段；
- 冻结 sidecar journal 的 ack/dispatch/observed/result 写入顺序、sync/corruption/disk-full/upgrade 不变量；
- 只传输或引用 #322 canonical Failure Envelope/receipt，不另造错误模型。
...
- dedicated-profile 真实账号 spike 已执行并记录采用/拒绝依据；
- 最终 artifact + production extension + 受控真实猎聘账号 canary 通过。
...
- 把专用 Chrome profile、Native Messaging 或 DBOS 的采用结果作为 v1 必选技术；其中 dedicated-profile spike 本身仍是 release evidence 必需项。
...
Real-site canary: PASS|FAIL + sanitized evidence
...
只有 `PR gate = PASS`、`Nightly gate = PASS`、所有承诺平台的 `Clean-machine release matrix = PASS`、`Real-site canary = PASS`、unknown/privacy blocker 为零，且没有开放的 release blocker 时，版本才可裁决为 production releaseable。
