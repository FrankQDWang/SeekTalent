...
- Windows signed admission additionally owns a live `WindowsOpenedInstalledRelease`: local NTFS only, complete installed-root-to-admitted-file `CreateFileW` handles with no write/delete sharing, reparse rejection, and stable `FILE_ID_INFO` plus final-handle-path evidence. Manifest、detached signature和sidecar executable的bytes/size/digest都来自这组仍打开的objects；不得在descriptor关闭后重新做path-only admission。直到后继suspended `CreateProcessW` adapter完成，Windows `spawn_owned_sidecar`固定返回`launch_binding_unsupported`并在child creation前释放所有handles/slot lease，绝不回退到`Popen`。
