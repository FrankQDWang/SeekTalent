...
- `AcceptedAck/Progress/Result/Failure/Query/Status/Cancel/Reconcile/Readiness/Drain` 消息；
...
Wire 可携带以下 opaque ref object；字段语义来自 owner 文档，本文不创建第二份对象：

- `FailureEnvelopeRef`：#322 `failure_id + revision`，可附 canonical hash；
- `ReceiptRef`：#322 receipt ID/revision/canonical hash 与 receipt kind；
- `OperationEvidenceRef`：#322 `operation_evidence_id + revision + canonical_hash`；
- `MainSemanticRefs`：main source-ledger revision 上的 `source_operation_disposition_ref`、`retry_posture_ref`，以及若 run 已 finalized 时的 `product_outcome_ref`。

Sidecar-originated first observation 的 `MainSemanticRefs` 必须全为 null。Main 先把 observation 通过 expected-revision transaction 提交为 durable business truth，才可在内部 provider adapter 关联 refs，或在后续 authenticated reconcile acknowledgement 中发送 refs。Sidecar最多原样保存/回显 main 已签发的 ref；它不能从 failure、timeout、HTTP code、candidate count 或 journal state计算 enum。
...
| `deadline` | `{value, clock, unit}`，定义见 §5.3 |

Main 在 durable source-operation accepted/outbox transaction 之前不能发送；port client 在写 frame 前用同一个 main transaction boundary/guard 重新验证 run、attempt、raw token、operation、request hash 和 expected ledger revision。Sidecar还要拒绝低于其已观察 run attempt high-watermark 的 request，以及同 attempt/operation 下 token ref 不一致的 request。它只保存 token 的 keyed non-reversible ref。
...
component_receipt_refs           # #322 refs only
operation_evidence_ref           # nullable #322 ref
failure_envelope_ref             # nullable #322 ref
main_semantic_refs               # null on first sidecar observation; §4.2
cleanup_fact                     # none | completed | incomplete, plus safe count/reason
result_payload_hash              # SHA-256 of canonical result payload
```

`side_effect_observation=unknown` 不等于 `failed`。Result 不含 `retryable/safe_to_retry/retry_after`。`stop_reason_code` 是 producer fact，只能经 #322 registry 映射；不能直接决定 state/outcome。
...
- Machine Capability/Startup Receipt refs；
...
|---|---|---|
| `operation.accepted_ack` | `seektalent.source-port.accepted-ack/v1` | identity without raw token、accepted generation、journal revision、`new|same_key_replay`、ack time、request hash |
...
| `operation.failure` | `seektalent.source-port.operation-failure/v1` | identity、journal phase/revision、side-effect observation、safe component/reason/counts、canonical refs |
...
|---|---|---|
| `operation.submit.<kind>` | main → sidecar | 完整 operation-specific request；sidecar先验证 frame/auth/schema/authority/idempotency/deadline/journal health |
...
- `operation.query`：按 `(operation_id, idempotency_key, request_hash, accepted_generation_hint)` 跨 generation 查询 identity/dedupe history。返回 `not_found|matched|conflict|history_unavailable`，以及 journal completeness/truncation facts。只有 journal integrity完整、generation range完整且该 generation无 accepted记录时，`not_found` 才能证明 sidecar未接受；否则是 unavailable，不能推出未 dispatch。
- `operation.status`：对已匹配 operation返回最新 durable phase `accepted|dispatch_intent|observed|reconciled`、generation、last command ordinal、result availability、cleanup fact。它不返回 `running/completed/failed` product status或 RetryPosture。
...
`operation.reconcile.inspect` 由 main no-owner recovery authority发送，不要求 active runtime bearer token；authenticated inherited-pipe session只能证明请求来自当前 main process，不是第二个 business token。Request携带 operation/key/hash、generation range、expected main ledger/reconciliation revisions和已知 dispatch/observation refs。
...
`port.readiness.query/result` 只报告：sidecar lifecycle、journal health、draining flag、supported operation contracts、component receipt refs、profile binding generation和 capability gap codes。它不代替 `verify_session` 的 account/search/risk事实。
...
明确排除：attempt number、raw runtime token/token ref、deadline、correlation ID、browser control scope ID、transport session/message/sequence、wall time、sidecar generation和main expected revisions。这样同一logical operation可在新runtime attempt中保持same hash，但不能跨profile/account binding generation透明续跑；每次dispatch仍需重新验证当前runtime/profile/browser authorities。

### 8.2 Required behavior

- same idempotency key + same request hash + same operation ID：返回同一 logical operation。若已 observed，返回相同 result hash/payload或 `already_reconciled + main_commit_ref`；不得再次产生 browser side effect。
...
- Same-key replay可以携带更新后的 attempt/token/deadline，但profile generation必须与hashed intent相同；sidecar先query旧record并执行authority/reconciliation gate，不能因为new token自动repeat。Binding generation变化时main先reconcile旧operation，再创建新的operation/key；不得透明换profile/account。
...
1. **MainHello**（唯一 unauthenticated frame）：protocol major/minor range、`product_version/product_build_id/main_build_id`、`release_manifest_ref`、supported operation contract map、frame caps、`session_id`、256-bit random `main_nonce`。
2. **SidecarHello**：echo session ID、exact `product_version/product_build_id`、`sidecar_build_id/executable_hash`、protocol range、operation contract map、journal schema/health、sidecar generation、256-bit random `sidecar_nonce`、hash of received MainHello。
3. 双方计算 `session_key = HKDF-SHA256(main_nonce || sidecar_nonce, salt=SHA256(product_build_id || session_id), info="seektalent-source-port-v1")`，并计算完整 hello transcript hash。Main先发 authenticated **MainReady**：chosen minor、contract map、transcript hash、initial direction seq。
4. Sidecar验证后发 authenticated **SidecarReady**。在双方 ready 前不得接受 operation、启动WTSCLI或产生browser side effect。

Main在spawn前按 #326 installed Release Manifest验证sidecar executable hash；raw nonce/session key不进入argv、environment、stderr、journal、receipt或support bundle。Inherited parent-child pipes + exact executable verification建立初始channel identity；HMAC保护post-handshake完整性、方向sequence和replay。

### 9.3 Frame authentication/replay

`auth_tag = HMAC-SHA256(session_key, direction_label || uint32_be(N_without_auth_tag) || canonical_json_without_auth_tag)`。

每方向 `direction_seq` 必须严格+1。Duplicate seq/message ID、gap、wrong reply、bad HMAC或old session frame触发typed protocol failure并关闭session；不能“尝试继续”。Reconnection总是新session/nonces/key；operation idempotency来自journal，不来自transport replay。
...
每个logical operation严格单向：

```text
accepted -> dispatch_intent -> observed -> reconciled
```

- `accepted`：identity/key/hash、accepted generation、safe authority refs、frame digest durable；完成后才发 Ack。
- `dispatch_intent`：每个browser command ordinal/ID/ref、profile generation、browser scope correlation、controller fence **ref only** durable；commit后才可发command。
- `observed`：conclusive/unknown observation、result spool hash/ref/size、safe counts/reason与canonical refs durable；commit后才发Result/Failure。
...
一次operation可有多个dispatch intent/observation event，但head phase不倒退；command ordinal严格递增。Observation不一定conclusive；unknown仍是observed external fact，不能伪造result。

### 10.3 SQLite transaction 与 sync policy

目标journal使用独立sidecar SQLite file：`journal_mode=WAL`、`synchronous=FULL`、`foreign_keys=ON`、bounded busy timeout。每个 critical transition：

1. `BEGIN IMMEDIATE`；
2. append immutable journal event；
3. expected-revision CAS update operation head/idempotency index；
4. commit；
5. 确认 commit/WAL sync 成功后才越过对应外部边界。

`accepted/dispatch_intent/observed/reconciled` 全部 FULL-sync。Progress不持久化。Protected result spool必须先atomic write、fsync file、fsync parent dir，再让`observed` transaction引用它。Drain执行bounded passive checkpoint并fsync；checkpoint失败不删除WAL，不报告drained success。
...
Journal总hard budget：50,000 immutable transition rows、64 MiB（含WAL/SHM）、14天event detail，任一先到触发deterministic compaction。
...
4. tombstone含operation/key/hash、first/last generation、result hash、main commit ref、reconciled revision；只有main在其source-operation ledger retention结束后发authenticated retention-release，才可删除；
...
- Migration采用new-file/copy/validate/fsync/atomic-switch并保留old backup；任何失败继续使用完整旧schema或not-ready，不自动删除/重建。
...
`FakeDuplexTransport` 必须可确定性注入：1-byte fragmentation、coalesced frames、header/body EOF、oversize length、invalid UTF-8/JSON、duplicate key、unknown field/type、blocked writer、stderr flood、seq duplicate/gap/reorder、bad HMAC、old session replay、wrong reply ID、hello transcript tamper和各compatibility mismatch。

断言：parser有固定memory cap；任何invalid input在journal/browser前拒绝；EOF/timeout只产生transport fact；reconnect使用new nonce/session且same operation靠journal dedupe。
...
在temporary real SQLite file、production journal mode/sync设置下对每个statement/commit/fsync前后注入exception或独立process kill：
...
- WAL/DB corruption、schema ahead、migration kill、FULL/READONLY/IOERR/BUSY均fail closed；故障解除后`integrity_check`通过；
- compaction/tombstone/retention-release crash保持idempotency；budget exhaustion不evict unresolved。

### 12.4 Operation contract goldens

六种request/result、全部message与每个minor compatibility组合有canonical JSON/hash golden。Property tests覆盖field/list/depth/frame limits、unknown field、request hash normalization、partial-with-records、zero result、per-item details partiality、cleanup failure与continuation ref binding。
...
- Failure Envelope/receipt/evidence只用#322 schema IDs/refs，本文没有shadow model；
...
- 实现本文framing/handshake/auth、六种DTO/message、journal、supervisor与fake/synthetic browser；
...
- sidecar以main-owned child handle启动；nonce不在argv/env/log；stdout protocol与stderr log独立drain；
- release/data/profile分目录；journal/result spool在product data root，不随binary rollback静默回退；migration/rollback mismatch fail closed；
...
| Frozen question | #322 | #324 | #325 resolution |
|---|---|---|---|
| canonical failure/evidence | 唯一schema/issuer/store | 只在main business transaction引用 | wire只携带canonical refs/producer facts，不复制schema |
...
未发现需要第二套权威语义的冲突。表面张力“sidecar返回RetryPosture”按main-authored ref处理：sidecar返回external facts，main按#324提交posture并可在后续ack/内部adapter关联ref；sidecar从不授予`safe_retry`。`accepted → dispatch_intent → observed → reconciled`同时存在于main ledger与sidecar journal，但owner不同：main是business interpretation，sidecar是external-effect evidence；两者通过authenticated refs/CAS关联，不双写同一truth。

## 17. Document Definition of Done

- [x] 六个production operation有独立request/result DTO，无generic command。
- [x] request identity、relative-monotonic deadline、三种authority/correlation边界已冻结。
- [x] ack/progress/result/failure/query/status/cancel/reconcile/readiness/drain已冻结。
- [x] same-key/same-hash replay与same-key/different-hash conflict已冻结。
- [x] journal durable order、FULL sync、generation lookup、retention、corruption/migration/disk-full fail-closed已冻结。
- [x] product/main/sidecar handshake、framing、caps、nonce/HMAC、compatibility与typed mismatch已冻结。
- [x] #322 evidence与#324 semantic refs只引用不重定义；controller secret不进入main。
