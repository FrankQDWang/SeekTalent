...
3. bootstrap 先生成 `5-8` 条 candidate seeds，再剪成 final seeds；generic 固定 `4` 条，routed 固定 `5` 条。
...
2. `[[RouteDomainKnowledgePack]]` 已落地为稳定纯函数，routing 固定为 `explicit_pack / inferred_single_pack / inferred_multi_pack / generic_fallback` 四选一；未知 explicit `knowledge_pack_id_override` 直接 fail-fast。bootstrap 所需的最小 domain knowledge packs 已以内置 fixture 形式落地。
...
4. `[[GenerateBootstrapOutput]]` 已落地为稳定纯函数；当前实现先生成 `5-8` 条 candidate seeds，再按强制保留和 Jaccard orthogonal prune 收口成 final seeds。generic fallback 固定 `4` 条，routed path 固定 `5` 条。补充说明：`BootstrapKeywordGenerationLLM` 的 round-0 输出已收窄到 `BootstrapKeywordDraft`，不再让 LLM 直接写 operator patch。
...
2. 已满足。routing 只有 `explicit_pack / inferred_single_pack / inferred_multi_pack / generic_fallback` 四种模式；当前测试已覆盖 explicit、single-pack、multi-pack、generic 四条路径以及相关 fail-fast 边界。
3. 已满足。bootstrap 先生成 `5-8` 条 candidate seeds；generic fallback 固定产出 `4` 条 final seeds，routed path 固定产出 `5` 条。
...
3. generic provenance 下不得放开 `pack_expansion / cross_pack_bridge`。
...
3. 已满足。generic provenance 下不会放开 `pack_expansion / cross_pack_bridge`；当前实现继续以 `knowledge_pack_ids == []` 作为唯一 generic provenance 判据。
...
- Phase 5 的实现仍坚持 `v0.3.1` 实验项目口径：不引入 compatibility shim、不增加 fallback/retry chain、不预埋 Phase 6 的 artifact writer。
